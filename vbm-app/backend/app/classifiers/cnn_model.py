"""
cnn_model.py — Loader del CNN (MedicalNet ResNet-18, TorchScript)
...
NOTA: el target_affine 4x4 con translation = 0 reproduce literalmente el código
...
def preprocess_gm_map(gm_map_path: Path) -> torch.Tensor:
    ...
    Args:
        gm_map_path: Ruta al mwp1*.nii.gz producido por deepmriprep
    ...
    # 1. Cargar imagen (soporta .nii y .nii.gz)
...
# ─── Métricas del modelo (fold 3 — mejor fold individual) ────────────────────
# Computado en Colab sobre 156 sujetos de validación (87 pac + 69 ctrl).
# CV stats son el promedio sobre los 5 folds (fold 0..4).
CNN_MODEL_METRICS = {
    # Fold 3 (representativo — mejor AUC individual)
    "auc":               0.7968,
    "sensitivity":       0.5581,
    "specificity":       0.8841,
